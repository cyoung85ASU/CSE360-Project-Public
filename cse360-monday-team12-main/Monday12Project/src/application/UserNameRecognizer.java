...
public class UserNameRecognizer {
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			// next state
			switch (state) {
			 // STATE 0: First character must be A-Z or a-z ONLY
            case 0:
            ...
            // STATE 1: We have at least one letter so far. Allow alphanumeric or special.
            // If special, go to State 2, stay in State 1 if alphanumeric.
            ...
            case 1:
                if ((currentChar >= 'A' && currentChar <= 'Z') ||
                    ...
                    // more alphanumeric => stay in state 1
                    nextState = 1;
                    userNameSize++;
                }
                else if (currentChar == '.' || currentChar == '-' || currentChar == '_') {
                    // special => go to state 2
                    nextState = 2;
                    userNameSize++;
                }
                else {
            ...
            // STATE 2: Next char MUST be alphanumeric, then go to State 1.
            case 2:
                if ((currentChar >= 'A' && currentChar <= 'Z') ||
                    (currentChar >= 'a' && currentChar <= 'z') ||
                    (currentChar >= '0' && currentChar <= '9')) {
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage += 
					"A UserName can only contain letters, digits, . - or _.\n";
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
		...
		case 2:
			// Need alphanumeric after special
			userNameRecognizerErrorMessage +=
				"After a special character (., -, _), you must have an alphanumeric next.\n";
			return userNameRecognizerErrorMessage;
